...
// For a Max-Heap, a child should be bubbled up if it's GREATER than its parent.
void heapifyUp(MaxHeap *heap, int index) {
    int parent = (index - 1) / 2; // Calculate parent index

    // Continue bubbling up as long as the current element is GREATER than its parent
...
// For a Max-Heap, a parent should be bubbled down if it's SMALLER than its largest child.
void heapifyDown(MaxHeap *heap, int index) {
    ...
    if (largest != index) {
        ...
        heapifyDown(heap, largest); // Recursively call heapifyDown on the swapped child
    }
}
...
// Extract (remove) the maximum element from the Max-Heap (which is always at the root)
int extractMax(MaxHeap *heap) {
    if (heap->size <= 0) {
        printf("Error: Heap is empty. Cannot extract maximum.\n");
    ...
    // Store the root (maximum) element to return later
    int root = heap->arr[0];
...
int peekMax(MaxHeap *heap) {
    if (heap->size <= 0) {
        printf("Error: Heap is empty. No maximum element.\n");
        return -1;
    }
...
int main() {
    MaxHeap myHeap;
    initMaxHeap(&myHeap); // Initialize the heap
    ...
    do {
        ...
        switch (choice) {
            ...
            case 4:
                ...
                break;
            case 5:
                printf("Exiting Max-Heap program. Goodbye!\n");
                break;
            default:
